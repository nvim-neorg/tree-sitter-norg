module.exports = grammar({
	...
	conflicts: $ => [
  	...
  	rules: {
		...
		todo_item_done: $ => seq(optional($._leading_whitespace), /\-\s+\[\s*x\s*\]\s+/, $.paragraph_segment),
		todo_item_pending: $ => seq(optional($._leading_whitespace), /\-\s+\[\s*\*\s*\]\s+/, $.paragraph_segment),
		todo_item_undone: $ => seq(optional($._leading_whitespace), /\-\s+\[\s+\]\s+/, $.paragraph_segment),

		tag: $ => seq(optional($._leading_whitespace), token(/@/), $.tag_name, repeat(seq(token.immediate('.'), $.tag_name)), choice(seq(/\s+/, seq(choice($.tag_parameters, $._soft_paragraph_break), repeat(seq(/[\t ]+/, $.tag_parameters)), $._soft_paragraph_break)), $._soft_paragraph_break), $.tag_content, '@end'),
		tag_content: $ => prec.left(1, repeat1(seq(/[^\n]+/, $._soft_paragraph_break))),
		tag_name: $ => /[a-z_]+/,
		tag_parameters: $ => /[^\s]+/,

		carryover_tag: $ => seq(optional($._leading_whitespace), token('$'), $.tag_name, repeat(seq(token.immediate('.'), $.tag_name)), choice(seq(/\s+/, seq(choice($.tag_parameters, $._soft_paragraph_break), repeat(seq(/[\t ]+/, $.tag_parameters)), $._soft_paragraph_break)), $._soft_paragraph_break), repeat($._soft_paragraph_break), repeat1($.paragraph)), 

		drawer: $ => seq(optional($._leading_whitespace), /\|{2}\s+/, field("drawer_name", $.paragraph_segment), optional($.drawer_content), '||'),
		drawer_content: $ => /.+/,
		...
		escape_sequence: $ => seq(optional($._leading_whitespace), token.immediate('\\'), token.immediate(/./)),
		words: $ => seq(choice(/[^\s]/, $._leading_whitespace), repeat1(/[^\n]/)),
